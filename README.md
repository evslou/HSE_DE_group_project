...
Для заполнения таблиц данными следует запустить в Airflow DAG с названием [parquet_to_postgres_loader]. Изначально при запуске будет 2 процесса параллельно отрабатывать. Один из них надо "убить", чтобы память не переполнилась от двух процессов параллельно. (process_and_load_data квадратик процесса -> "Mark state as..." -> failed). 

Ожидание отработки процесса зависит от мощности компа, но где-то 10-30 минут. Можно отслежить процесс загрузки через таблицу processed_files в airflow_db (всего должно быть загружено 35 файлов)
